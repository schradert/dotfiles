...
in {
  perSystem.canivete.pre-commit.settings.hooks.lychee.toml.exclude = ["https://download.tlopo.com/*"];
  dotfiles.home-manager = {
    ...
    config = lib.mkIf config.dotfiles.profiles.gaming.enable {
      dotfiles.programs.steam.external.manual."The Legend of Pirates Online".shortcut.exe = lib.getExe (inputs.erosanix.lib.${pkgs.system}.mkWindowsApp rec {
      ...
      # NOTE also being used in ./wizard101.nix
      home.activation.tlopo-controls = lib.hm.dag.entryAfter ["writeBoundary"] ''
        ...
        src=${(pkgs.formats.json {}).generate "tlopo.controls.json" controls}
