...
set ruler
set tabstop=2
set window=1154751091955466240
...
badd +1 install.sh
badd +0 vim/.vimrc.jeff
badd +1 vim/.vimrc
badd +1 git/.gitconfig
badd +0 vscodium/settings.json
argglobal
%argdel
edit vim/.vimrc
...
wincmd w
argglobal
if bufexists(fnamemodify("vim/.vimrc.jeff", ":p")) | buffer vim/.vimrc.jeff | else | edit vim/.vimrc.jeff | endif
balt install.sh
nnoremap <buffer> ,c I" 
vnoremap <buffer> <silent> [" :exe "normal! gv"|call search('\%(^\s*".*\n\)\%(^\s*"\)\@!', "bW")
nnoremap <buffer> <silent> [" :call search('\%(^\s*".*\n\)\%(^\s*"\)\@!', "bW")
