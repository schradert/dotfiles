...
eof
  ...
  # Edited this line as per https://github.com/mbhynes/fzf-gcloud/issues/9
  # gcloud_bin_path="$(which gcloud)"
  gcloud_root="$(gcloud info 2>/dev/null |grep -i 'Installation Root' | awk -F '[][]' '{print $2}')"
  gcloud_bin_path="$gcloud_root/bin/gcloud"
...
__gcloud_sel() {
  ...
  # gcloud_bin_path="$(which gcloud)"
  gcloud_root="$(gcloud info 2>/dev/null |grep -i 'Installation Root' | awk -F '[][]' '{print $2}')"
  gcloud_bin_root="$gcloud_root/bin"
