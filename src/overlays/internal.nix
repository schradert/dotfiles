final: prev: {
  ...
  in
    prev.runCommand "from-json-to-yaml" {nativeBuildInputs = [prev.remarshal];} command;
  toYAMLFile = obj: let
    file = builtins.toFile "obj.json" (builtins.toJSON obj);
  in
    final.fromJSONtoYAML file;
  toYAML = obj: [(builtins.readFile (final.toYAMLFile obj))];
  fromYAML = yaml: let
    command = "remarshal -if yaml -i \"${yaml}\" -of json -o \"$out\"";
  in
    prev.lib.trivial.importJSON (prev.runCommand "from-yaml" {nativeBuildInputs = [prev.remarshal];} command);
  subTemplateCmds = {
    template,
    cmds ? {},
  }: let
  ...
  in
    prev.lib.trivial.throwIf failed errors pkg;
  map' = values: function: builtins.map function values;
  mk-nux-pkg = module:
    prev.lib.trivial.pipe module [
      (mod:
        prev.lib.evalModules {
          specialArgs.pkgs = final;
          modules = [mod];
        })
