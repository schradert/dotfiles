{
  ...
  imports = [];
  options = {
    etcd.instances = lib.mkOption {
      default = 3;
      type = lib.types.int;
      description = "Number of etcd instances";
    };
    etcd.memory = lib.mkOption {
      default = 512;
      type = lib.types.int;
      description = "RAM per instance (MB)";
    };
    control-plane.instances = lib.mkOption {
      default = 3;
      type = lib.types.int;
      description = "Number of etcd instances";
    };
    control-plane.memory = lib.mkOption {
      default = 512;
      type = lib.types.int;
      description = "RAM per instance (MB)";
    };
    worker.instances = lib.mkOption {
      default = 2;
      type = lib.types.int;
      description = "Number of etcd instances";
    };
    worker.memory = lib.mkOption {
      default = 1024;
      type = lib.types.int;
      description = "RAM per instance (MB)";
    };
    load-balancer.instances = lib.mkOption {
      default = 2;
      type = lib.types.int;
      description = "Number of etcd instances";
    };
    load-balancer.memory = lib.mkOption {
      default = 512;
      type = lib.types.int;
      description = "RAM per instance (MB)";
    };
    ...
    resource =
      {
        libvirt_network.k8s = {
          ...
          dns.enabled = true;
          xml.xslt = let
            inherit ((import ../lib/shared.nix {inherit pkgs;}).funcs) templateWithArgs;
          in
            templateWithArgs ./xslt-dhcp.xml {
              dhcp_range_start = "10.25.0.1";
              dhcp_range_end = "10.25.0.24";
            };
        };
      }
      // (
