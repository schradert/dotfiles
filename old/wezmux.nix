{
  ...
  computers = [
    ...
    {
      hostname = "Daniels-iPhone";
      username = "tristanschrader";
      home_d = "/Users";
    }
  ];
  computersIndexed = listToAttrs (map (c: {
      name = c.hostname;
      value = c;
    })
    computers);
  ...
  cmds.run-pane-cmd = s: pi: cmd: wn: "tmux send-keys -t ${getPaneName s wn pi} '[[ -d ${s.cwd}/${wn} ]] && cd ${s.cwd}/${wn}; ${cmd}' Enter";
  ...
  '';
  bin.write-script = name: text:
    (pkgs.writeScriptBin name text).overrideAttrs (old: {
  ...
  sessions = [
    ...
    {
      name = "hive";
      path = "Personal/electrichive";
    }
    {
      name = "terrace";
      path = "Personal/terrace";
    }
    {
      name = "couchers";
      path = "Personal/couchers";
    }
  ];
  getHostSessions = hn:
    map (s:
      s
      // {
        cwd = getHostSessionFolder hn s;
        windows = cmds.list-windows hn s;
      })
    sessions;
