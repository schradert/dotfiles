{inputs, ...}: {
  dotfiles.nodes.systeamadeck.system.imports = [
    inputs.mynur.nixosModules.decky
    {
      # Decky Loader
          ...
          config = lib.mkMerge [
            {
              ...
              in
                lib.hm.dag.entryAfter ["writeBoundary"] ''
                  appid=$(${lib.getExe pkgs.nostatoo} list-non-steam-games | ${pkgs.gawk}/bin/awk -F': ' '/Battle.Net/ {print $2}')
