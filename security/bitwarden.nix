{
  dotfiles = {
    ...
    subdomain = "bitwarden.${config.domain}";
    image = {
      imageName = "";
      imageDigest = "";
      hash = "";
  ...
  in {
    options.services.bitwarden.enable = mkEnableOption "bitwarden";
    config = mkMerge [
      {
        darwin.homebrew.casks = ["bitwarden"];
        home-manager = {pkgs, ...}: {
          home.packages = mkIf pkgs.stdenv.hostPlatform.isLinux [pkgs.bitwarden];
          programs.rbw.enable = true;
          # TODO pass email, base_url, identity_url
        };
      }
      (mkIf config.services.bitwarden.enable {
        nixos = {pkgs, ...}: {canivete.kubernetes.images.bitwarden = pkgs.dockerTools.pullImage image;};
        kubenix = {helm, ...}: {
          namespace = "security";
          chart = helm.fetch {
          ...
          values.component.scim.enabled = true;
          values.secrets.secretName = "bitwarden";
          extraResources = {
            secrets.bitwarden.stringData = {
            ...
            # configMaps.bitwarden-config-map.data.ACCEPT_EULA = mkForce "'Y'";
            # TODO Routing
